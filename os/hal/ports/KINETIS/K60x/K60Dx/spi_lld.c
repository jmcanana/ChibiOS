...
#define DMAMUX_SPI0_RX_SOURCE    16
#define DMAMUX_SPI0_TX_SOURCE    17
...
#define KINETIS_SPI1_RX_DMAMUX_CHANNEL      2
#endif

#if !defined(KINETIS_SPI1_RX_DMA_CHANNEL)
#define KINETIS_SPI1_RX_DMA_CHANNEL         2
#endif

#if !defined(KINETIS_SPI1_TX_DMAMUX_CHANNEL)
#define KINETIS_SPI1_TX_DMAMUX_CHANNEL      3
#endif

#if !defined(KINETIS_SPI1_TX_DMA_CHANNEL)
#define KINETIS_SPI1_TX_DMA_CHANNEL         3
#endif

#define DMAMUX_SPI1_RX_SOURCE    18
#define DMAMUX_SPI1_TX_SOURCE    19

#if !defined(KINETIS_SPI_USE_SPI2)
#define KINETIS_SPI_USE_SPI2                TRUE
#endif

#if !defined(KINETIS_SPI2_RX_DMA_IRQ_PRIORITY)
#define KINETIS_SPI2_RX_DMA_IRQ_PRIORITY    8
#endif

#if !defined(KINETIS_SPI2_RX_DMAMUX_CHANNEL)
#define KINETIS_SPI2_RX_DMAMUX_CHANNEL      4
#endif

#if !defined(KINETIS_SPI2_RX_DMA_CHANNEL)
#define KINETIS_SPI2_RX_DMA_CHANNEL         4
#endif

#if !defined(KINETIS_SPI2_TX_DMAMUX_CHANNEL)
#define KINETIS_SPI2_TX_DMAMUX_CHANNEL      5
#endif

#if !defined(KINETIS_SPI2_TX_DMA_CHANNEL)
#define KINETIS_SPI2_TX_DMA_CHANNEL         5
#endif

#define DMAMUX_SPI2_RX_SOURCE    20
#define DMAMUX_SPI2_TX_SOURCE    21
...
spiDmaResources_t spiDmaResources[] = {
    [0] = {
        rxDmaIrqPriority = KINETIS_SPI0_RX_DMA_IRQ_PRIORITY,

        rxDmaMuxChannel  = KINETIS_SPI0_RX_DMAMUX_CHANNEL,
        rxDmaChannel     = KINETIS_SPI0_RX_DMA_CHANNEL,
        rxDmaSource      = DMAMUX_SPI0_RX_SOURCE,

        txDmaMuxChannel  = KINETIS_SPI0_TX_DMAMUX_CHANNEL,
        txDmaChannel     = KINETIS_SPI0_TX_DMA_CHANNEL,
        txDmaSource      = DMAMUX_SPI0_TX_SOURCE,
    },
    [1] = {
        rxDmaIrqPriority = KINETIS_SPI1_RX_DMA_IRQ_PRIORITY,

        rxDmaMuxChannel  = KINETIS_SPI1_RX_DMAMUX_CHANNEL,
        rxDmaChannel     = KINETIS_SPI1_RX_DMA_CHANNEL,
        rxDmaSource      = DMAMUX_SPI1_RX_SOURCE,

        txDmaMuxChannel  = KINETIS_SPI1_TX_DMAMUX_CHANNEL,
        txDmaChannel     = KINETIS_SPI1_TX_DMA_CHANNEL,
        txDmaSource      = DMAMUX_SPI1_TX_SOURCE,
    },
    [2] = {
        rxDmaIrqPriority = KINETIS_SPI2_RX_DMA_IRQ_PRIORITY,

        rxDmaMuxChannel  = KINETIS_SPI2_RX_DMAMUX_CHANNEL,
        rxDmaChannel     = KINETIS_SPI2_RX_DMA_CHANNEL,
        rxDmaSource      = DMAMUX_SPI2_RX_SOURCE,

        txDmaMuxChannel  = KINETIS_SPI2_TX_DMAMUX_CHANNEL,
        txDmaChannel     = KINETIS_SPI2_TX_DMA_CHANNEL,
        txDmaSource      = DMAMUX_SPI2_TX_SOURCE,
    },
};
/*===========================================================================*/
...
#if KINETIS_SPI_USE_SPI2 || defined(__DOXYGEN__)
SPIDriver SPID3;
#endif
...
void spi_lld_start(SPIDriver *spip) {
  ...
  if (spip->state == SPI_STOP) {
...
#endif
#if KINETIS_SPI_USE_SPI2
    if (&SPID3 == spip) {
        spip->config->spiBus = 2;

        /* Enable the clock for SPI0 */
        SIM->SCGC3 |= SIM_SCGC2_SPI2;
    ...
    nvicEnableVector(DMA0_IRQn, spiDmaResources[spip->config->spiBus].
                                rxDmaIrqPriority);
    ...
    DMAMUX->CHCFG[spiDmaResources[spip->config->spiBus].rxDmaMuxChannel]
        = DMAMUX_CHCFGn_ENBL
        | DMAMUX_CHCFGn_SOURCE(
                            spiDmaResources[spip->config->spiBus].rxDmaSource);

    /* Tx, select SPI Tx FIFO */
    DMAMUX->CHCFG[spiDmaResources[spip->config->spiBus].txDmaMuxChannel]
        = DMAMUX_CHCFGn_ENBL
        | DMAMUX_CHCFGn_SOURCE(
                             spiDmaResources[spip->config->spiBus].txDmaSource);
    ...
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].SADDR
                                                    = (uint32_t)&SPI0->POPR;
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].SOFF     = 0;
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].SLAST    = 0;
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].DLASTSGA = 0;
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].ATTR
                                                    = DMA_ATTR_SSIZE(dma_size) |
                                                      DMA_ATTR_DSIZE(dma_size);
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].NBYTES_MLNO
                                                    = spip->word_size;
    DMA->TCD[spiDmaResources[spip->config->spiBus].rxDmaChannel].CSR
                                                    = DMA_CSR_DREQ_MASK |
                                                      DMA_CSR_INTMAJOR_MASK;

    /* configure DMA TX fixed values */
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].SLAST = 0;
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].DADDR
                                                       = (uint32_t)&SPI0->PUSHR;
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].DOFF     = 0;
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].DLASTSGA = 0;
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].ATTR
                                                    = DMA_ATTR_SSIZE(dma_size) |
                                                      DMA_ATTR_DSIZE(dma_size);
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].NBYTES_MLNO
                                                    = spip->word_size;
    DMA->TCD[spiDmaResources[spip->config->spiBus].txDmaChannel].CSR
                                                    = DMA_CSR_DREQ_MASK;
  }
}
...
void spi_lld_stop(SPIDriver *spip) {
  ...
  if (spip->state == SPI_READY) {
    ...
    /* Disable the clock for SPI2 */
    SIM->SCGC3 &= ~SIM_SCGC3_SPI2;
